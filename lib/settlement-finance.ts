...
export interface BookingFinance {
  ...
  collectedVnd: bigint;
  /** 수납액의 VND 환산 — KRW는 스냅샷 환율로 환산, VND는 그대로. 환율 미상이면 null */
  collectedVndEquivalent: bigint | null;
  /** 공급자 지급액 (VND) */
  payoutVnd: bigint;
  /** 운영자 마진 = 수납 VND환산 − 지급. 환율 미상이면 null */
  marginVnd: bigint | null;
  /** KRW 예약인데 환율 스냅샷이 없어 환산 불가 */
  fxMissing: boolean;
}

/** 지원 통화 화이트리스트 — 그 외(USD 등)는 정산 손익 분기에 도달하면 throw (money-pattern 교훈) */
function assertSupportedCurrency(c: Currency): void {
  if (c !== Currency.KRW && c !== Currency.VND) {
