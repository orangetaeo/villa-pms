...
describe("환산·환산 후 마진 (KRW를 예약 스냅샷 환율로 VND 환산)", () => {
  ...
  it("미니바·서비스는 USD 없음 (saleUsd=null)", () => {
    expect(
      buildMinibarTxn({
        id: "m", bookingId: "bk-m", checkOut: D("2026-07-05"), villaId: "v1", villaName: "V12",
        nameKo: "콜라", consumedQty: 1, lineVnd: 20_000n, lineCostVnd: 10_000n,
      }).saleUsd
    ).toBeNull();
  });

  it("USD 행 + 환율(25400) → saleUsd=1500, 환산=38,100,000, marginVnd=환산−원가", () => {
    const t = buildRoomTxn({
      ...krwBase,
    ...
    expect(t.saleUsd).toBe(1_500);
    expect(t.saleKrw).toBeNull();
    expect(t.saleVnd).toBeNull();
    expect(t.saleVndEquivalent).toBe(38_100_000n); // 1500 × 25400
    expect(t.marginVnd).toBe(33_100_000n); // 38,100,000 − 5,000,000
    expect(t.fxMissing).toBe(false);
  });

  it("USD 행인데 USD 환율 스냅샷 없음 → fxMissing=true, 환산·마진 null", () => {
    const t = buildRoomTxn({
      ...krwBase,
      saleCurrency: Currency.USD,
      totalSaleKrw: null,
  ...
  it("loadRevenueTxns: USD 예약 → saleUsd 원본·스냅샷 환율 환산 통합 매출 포함", async () => {
    const { db } = mockDb({
      bookings: [
        {
          id: "b-usd", checkOut: D("2026-07-10"), villaId: "v1",
          channel: BookingChannel.DIRECT, agencyName: null, guestName: "John",
          saleCurrency: Currency.USD, totalSaleKrw: null, totalSaleVnd: null,
          totalSaleUsd: 1_500, supplierCostVnd: 5_000_000n,
          fxVndPerKrw: null, fxVndPerUsd: "25400",
          villa: { name: "V12" }, partner: null,
        },
      ],
    });
    ...
    expect(txns[0].saleUsd).toBe(1_500);
    expect(txns[0].saleVndEquivalent).toBe(38_100_000n);
    expect(totals.saleUsd).toBe(1_500); // 원본 USD 합
    expect(totals.saleKrw).toBe(0);
    expect(totals.saleVnd).toBe(0n);
    expect(totals.integratedRevenueVnd).toBe(38_100_000n);
    expect(totals.marginVnd).toBe(33_100_000n);
    expect(totals.fxMissingCount).toBe(0);
  });

  it("loadRevenueTxns: USD 예약인데 fxVndPerUsd 스냅샷 없음 → fxMissing 집계", async () => {
    const { db } = mockDb({
      bookings: [
        {
          id: "b-usd2", checkOut: D("2026-07-10"), villaId: "v1",
          channel: BookingChannel.DIRECT, agencyName: null, guestName: "Jane",
          saleCurrency: Currency.USD, totalSaleKrw: null, totalSaleVnd: null,
          totalSaleUsd: 1_000, supplierCostVnd: 5_000_000n,
          fxVndPerKrw: null, fxVndPerUsd: null,
          villa: { name: "V12" }, partner: null,
        },
      ],
    });
    const { totals } = await loadRevenueTxns(
      ...
      { from: D("2026-07-01"), to: D("2026-08-01"), types: ["ROOM"] },
      undefined,
      "18" // KRW 폴백은 USD에 영향 없음(스냅샷 우선·폴백 없음)
    );
    expect(totals.saleUsd).toBe(1_000);
    expect(totals.integratedRevenueVnd).toBe(0n); // 환산 불가
    expect(totals.fxMissingCount).toBe(1);
  });

  it("currency=USD 필터: USD 매출 행만 통과", async () => {
    const { db } = mockDb({
      bookings: [
        {
          id: "b-usd", checkOut: D("2026-07-10"), villaId: "v1",
        ...
        {
          id: "b-vnd", checkOut: D("2026-07-11"), villaId: "v2",
          channel: BookingChannel.DIRECT, agencyName: null, guestName: "Nguyen",
          saleCurrency: Currency.VND, totalSaleKrw: null, totalSaleVnd: 8_000_000n,
          totalSaleUsd: null, supplierCostVnd: 5_000_000n,
          fxVndPerKrw: null, fxVndPerUsd: null,
          villa: { name: "V11" }, partner: null,
        },
      ],
    });
    const { txns } = await loadRevenueTxns(db, {
      from: D("2026-07-01"), to: D("2026-08-01"), types: ["ROOM"], currency: Currency.USD,
    });
    expect(txns).toHaveLength(1);
    expect(txns[0].id).toBe("ROOM:b-usd");
  });
});
