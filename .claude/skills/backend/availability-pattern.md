...
- (T2.3 QA, 2026-06-11) **락 키 단위보다 넓은 불변식은 advisory lock으로 못 지킨다** — 빌라 단위 락은 "한 제안에 HOLD 1건" 같은 제안 레벨 불변식을 보호하지 못함(같은 제안의 다른 빌라 item 동시 가예약 → 서로 다른 락 → 둘 다 통과). 이런 불변식은 상태 컬럼 원자 가드(`updateMany({where: {id, status: 기대값}})` + count 검사 throw)로 지킬 것 — READ COMMITTED에서 UPDATE는 행 잠금 후 WHERE를 최신 커밋 기준 재평가하므로 race 차단. 잠금은 `lib/availability.ts lockVillaInventory(tx, villaId)` 공용 헬퍼 사용(전 세션 동일 키 보장)
