...
| 티어 | 모델 | 담당 | 에이전트 |
|---|---|---|---|
| 최상위 (~5-10%) | **Fable 5** | 메인 세션 오케스트레이션, 아키텍처·스키마 결정, 난해한 디버깅, 보안·권한누수 전수감사 | TDA (+ 필요 시 QA를 `model: fable`로 오버라이드) |
| 표준 (~80%) | **Opus 4.8** | 코드 작성·수정, QA 검증, 배포, 현지화, 디자인 | BE·FE·UX-VN·INTEG·QA·OPS·LOC·DESIGN |
| 경량 | **Haiku 4.5** | 파일 탐색, 로그 파싱, 단순 분류, 문서·비용 집계 | PM·FIN·Explore(`.claude/agents/explore.md`가 내장 Explore를 haiku로 오버라이드 — 공식 문서 권장 패턴) |

- 메인 세션(Fable)은 직접 대량 코드 작성·광범위 탐색을 피하고 위임한다 — 판단·설계·병합만 직접.
- 승격 규칙: Opus 에이전트가 같은 문제를 2회 이상 실패하면 Fable로 오버라이드해 재시도.
