...
describe("투어 정의 무결성", () => {
  ...
  it("화면당 스텝은 3개 이하다 (관리자 상세 화면만 ≤6 완화 — T-7 계약)", () => {
    // 상세 화면은 기능 밀도가 높아 계약이 명시적으로 완화(테오 7기능 커버). vi 포털 3스텝 상한은 유지.
    const STEP_CAP_EXCEPTIONS: Partial<Record<TourId, number>> = {
      villaDetail: 6,
      bookingDetail: 6,
      partnerDetail: 5,
      supplierVillaDetail: 5,
      partnerBookingDetail: 4,
    };
    for (const [tourId, tour] of Object.entries(TOURS)) {
      const cap = STEP_CAP_EXCEPTIONS[tourId as TourId] ?? 3;
      expect(tour.steps.length, `${tourId} 스텝 수`).toBeLessThanOrEqual(cap);
      expect(tour.steps.length, `${tourId} 스텝 없음`).toBeGreaterThan(0);
    }
  });

  it("앵커 id는 투어 안에서 중복되지 않는다 (villaDetail의 의도적 1건 예외)", () => {
    // villaDetail vdetail-tab-overview 2회 = 같은 탭의 두 주제(요금/청소)를 인접 스텝으로 —
    // 하이라이트 고정 채 문구만 진행(FE 회의 확정). 그 외 중복은 실수이므로 가드 유지.
    const DUP_ALLOWED: Partial<Record<TourId, number>> = { villaDetail: 1 };
    for (const [tourId, tour] of Object.entries(TOURS)) {
      const anchors = tour.steps.map((s) => s.anchor);
      const dups = anchors.length - new Set(anchors).size;
      expect(dups, `${tourId} 앵커 중복`).toBeLessThanOrEqual(
        DUP_ALLOWED[tourId as TourId] ?? 0
      );
    }
  });
  ...
  it("route 매핑은 pathname 정확일치로 동작한다", () => {
    ...
    // /earnings는 T-6에서 투어 등록됨 — 무투어 경로 검증은 /profile로 대체
    expect(tourIdForRoute("/earnings")).toBe("earnings");
    expect(tourIdForRoute("/profile")).toBeNull();
  });
});
...
describe("isRectHorizontallyVisible — 반응형 이중 앵커 가시 판정 (순수 함수)", () => {
  const VW = 390;

  it("정상 가시 요소 → true", () => {
    expect(
      isRectHorizontallyVisible({ width: 100, height: 40, left: 10, right: 110 }, VW)
    ).toBe(true);
  });

  it("display:none(전부 0 rect) → false — 데스크톱 전용 요소를 모바일에서 스킵", () => {
    expect(isRectHorizontallyVisible({ width: 0, height: 0, left: 0, right: 0 }, VW)).toBe(false);
  });

  it("모바일 드로어 -translate-x-full(right=0 경계값 포함) → false", () => {
    expect(
      isRectHorizontallyVisible({ width: 256, height: 800, left: -256, right: 0 }, VW)
    ).toBe(false);
  });

  it("오른쪽 화면 밖(left ≥ viewport) → false", () => {
    expect(
      isRectHorizontallyVisible({ width: 100, height: 40, left: 400, right: 500 }, VW)
    ).toBe(false);
  });

  it("수직 오프스크린은 가시 취급(스크롤로 데려옴) → true", () => {
    // top/bottom은 판정에 없음 — 수평만 본다
    expect(
      isRectHorizontallyVisible({ width: 100, height: 40, left: 10, right: 110 }, VW)
    ).toBe(true);
  });
...
describe("data-tour 앵커 실존 — 정의된 앵커가 대상 화면 소스에 있다", () => {
  ...
  const ANCHOR_SOURCES: Record<TourId, string[]> = {
    ...
    villaDetail: [
      "app/(admin)/villas/[id]/page.tsx",
      "app/(admin)/villas/[id]/detail-tabs.tsx",
    ],
    bookingDetail: [
      "app/(admin)/bookings/[id]/page.tsx",
      "app/(admin)/bookings/[id]/action-panel.tsx",
    ],
    partnerDetail: ["app/(admin)/partners/[id]/partner-detail.tsx"],
    supplierVillaDetail: ["app/(supplier)/my-villas/[id]/page.tsx"],
    partnerBookingDetail: [
      "app/partner/bookings/[id]/page.tsx",
      "components/partner/partner-change-request-panel.tsx",
    ],
    adminProposals: ["app/(admin)/proposals/proposals-list.tsx"],
    adminInspections: ["app/(admin)/inspections/inspections-view.tsx"],
    adminAvailability: ["app/(admin)/availability/board-client.tsx"],
    adminSettlements: ["app/(admin)/settlements/settlements-view.tsx"],
    adminReceivables: ["app/(admin)/receivables/page.tsx"],
    adminServiceOrders: ["app/(admin)/service-orders/service-orders-view.tsx"],
    adminRevenue: ["app/(admin)/revenue/revenue-client.tsx"],
    adminStatistics: ["app/(admin)/statistics/statistics-client.tsx"],
    adminInventory: ["app/(admin)/inventory/inventory-client.tsx"],
    adminUsers: ["app/(admin)/users/users-manager.tsx"],
    adminPartners: ["app/(admin)/partners/partners-manager.tsx"],
    adminSettings: ["app/(admin)/settings/page.tsx"],
    earnings: [
      "app/(supplier)/earnings/page.tsx",
      "components/supplier/stats/stats-section.tsx",
    ],
    myBookings: ["app/(supplier)/my-bookings/page.tsx"],
    vendorStats: ["app/vendor/stats/page.tsx"],
  };
