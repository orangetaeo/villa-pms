...
// 협의(S2) — sign 라우트의 서명 게이트·mine의 이력 조회가 사용. 기본은 "협의 없음"(beforeEach).
const neg = {
  findFirst: vi.fn(),
  findMany: vi.fn(),
};
vi.mock("@/lib/prisma", () => ({
  prisma: {
    ...
    contractNegotiation: {
      ...
      findMany: (...a: unknown[]) => neg.findMany(...a),
    },
    user: {
...
beforeEach(() => {
  ...
  Object.values(neg).forEach((f) => f.mockReset());
  neg.findFirst.mockResolvedValue(null); // 미해결 협의 없음 = 서명 가능(기존 시나리오 유지)
  neg.findMany.mockResolvedValue([]);
});
...
describe("상대방 POST /business-contracts/[id]/sign (스코프·멱등·레이스)", () => {
  ...
  it("★ 미해결 협의가 있으면 서명 차단 409 NEGOTIATION_OPEN (S2 게이트)", async () => {
    mockAuth.mockResolvedValue(SUPPLIER);
    bc.findFirst.mockResolvedValue({ id: "c1", type: "VILLA_SUPPLY", locale: "vi", status: "SENT", termsJson: {} });
    neg.findFirst.mockResolvedValue({ id: "n1" }); // OPEN 협의 존재
    const res = await sign(signReq({ signName: "A", idNumber: "1", address: "addr" }), P("c1"));
    expect(res.status).toBe(409);
    expect(await res.json()).toMatchObject({ error: "NEGOTIATION_OPEN" });
    expect(bc.updateMany).not.toHaveBeenCalled(); // 서명 전이 시도조차 없음
  });
