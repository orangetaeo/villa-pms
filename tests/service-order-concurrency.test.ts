// 부가서비스 발주 동시성 가드 회귀 테스트 (debug-sweep)
...
describe("dispatch 동시성 가드 (이중 발주 차단)", () => {
  const baseOrder = {
    ...
    catalogItemId: "ci-1",
    vendorName: null,
    vendor: { id: "v-1", name: "V", userId: "vu-1", approvalStatus: "APPROVED", user: { zaloUserId: "z-1" } },
...
describe("vendor respond 동시성 가드 (이중 응답 차단)", () => {
  ...
  it("count===0 이면 409 이고 운영자 통지를 보내지 않는다", async () => {
    ...
    const res = await RESPOND(jsonReq({ accept: true }), {
