...
| A2 | **공개 직전 테스트 데이터 삭제** | 파일럿(A1) 완료 후 실행 | 스크립트 완료: `npx tsx scripts/cleanup-test-data.ts`(감사) → `--confirm`(안전 삭제, 거래 달린 건 자동 거부) |

> ✅ 이미 완료된 차단요인: Zalo QR 로그인(시스템봇 연결), 파일럿 시드 프로덕션 실행.
> **실질 잔여 차단 = A1 하나.** (A2는 검증·청소 단계)

---

## B. Zalo 라이브 실연결·실송수신 검증 (코드 완료, 실동작 확인 대기)

QR 로그인은 완료. 아래는 "실제로 메시지가 오가는지" 확인이 남은 항목.

| # | 항목 | 남은 일 | 계약/근거 |
|---|---|---|---|
| B1 | F5 알림 실발송 1회 + /messages 실수신 검증 | 실 송수신 1회 확인 | TASKS T3.5b·T6.6 |
| B2 | notifications cron Railway 등록 | `/api/cron/notifications` 5분 주기 등록(ical-sync 패턴) | TASKS T3.5 잔여 ②, LAUNCH §6.2 |
| B3 | T3.7 수신 webhook·전화번호 자동 매칭 실연결 | 라이브 활성화 시 동작 확인 | `docs/contracts/T6.6-T3.7-zalo-screens.md` |
| B4 | 여권 이미지 실발송 (T3.5b S5) | 토큰 후 실 이미지 발송 | TASKS T3.5b |
...
| # | 항목 | 비고 |
|---|---|---|
| D1 | 운영자 통계 화면 구현 | **다른 세션 진행 중**(worktree `wt/statistics`). 기획+Stitch 완료, 라우트 미구현 |
| D2 | T7.2 잔여 — 시스템 미러 귀속(S4)·끊김 경보(S6) | zalo 영역. 다른 세션이 통화 진단 작업 중 → 회피 |
