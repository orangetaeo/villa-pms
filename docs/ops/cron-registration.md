...
| `cron-youtube-edit-jobs` | `/api/cron/youtube-edit-jobs` | `*/5 * * * *` | **5분 간격** | ★유튜브 편집 잡 **주 실행 경로**(안전망 아님 — run 라우트는 큐잉만 함). 1회 1건 렌더(2.5~8분)·PROCESSING 25분 고아 회수·잡 0건=no-op. 주기 15→5분 상향(villa-clip-narration-p2, 2026-07-22). ★가장 자주 도는 "마케팅 잡 틱"이라 **발행 축 고아(InstagramPost·YoutubeShort의 status=PUBLISHING 45분 초과) 회수도 여기서 같이 돈다** — 별도 cron 없이 5분 내 감지, 응답 `publishOrphansReaped`(T-publish-orphan-reaper, 2026-07-23) |
...
| `cron-webchat-unanswered` | `/api/cron/webchat-unanswered` | `*/10 * * * *` | 10분마다 | **웹 채팅 미응답 리마인드**(A-17) — 마지막 메시지가 방문자(INBOUND)이고 `WEBCHAT_UNANSWERED_MINUTES`(기본 30분) 경과한 OPEN 세션 → 운영자 Zalo 그룹방 리마인드. 미응답 구간당 1회 멱등(DB 알림 이력 dedup), 대상 0건=no-op. 킬스위치 `ZALO_OPERATOR_NOTIFY_PAUSED` 상속. **미등록 시 신규 문의 알림만 가고 미응답 리마인드는 안 나감** (T-webchat-unanswered-reminder) |
