...
각 `Payment`(B2C purpose) PAID → **LEDGER에 VND로 전기**(자기 `fxRateToVnd`로 환산, `postCollection`). 다통화·
다시점 입금을 VND 기준으로 합산 → 매출·마진·정산 전부 VND 단일 기준(현금주의 유지). 공급자 정산은 VND 그대로.
...
3. **취소·환불 = "낸 통화·낸 금액 그대로 반환".** 각 `Payment`의 `amount`(낸 실금액)를 그 통화로 반환,
   LEDGER는 그 결제의 VND 전기를 **역분개**(재환산 없음 → 장부 정합). 부분환불=낸 금액의 취소규정 %만큼(§7).
   환불 `Payment`(음수 `amount`) 또는 별도 반환 기록 — 구현 시 `PaymentPurpose` 재사용 여부 결정.
