...
| `depositRatePct` | 계약금율(기본 30) — 정책 기본, 예약별 오버라이드 가능 |
...
## 8. 미해결 — 후속 결정 필요 (구현 스프린트 전 테오 확정)

1. **계약금율** 기본값(30%? 빌라·시즌별?).
2. **잔금 청구 시점** — 체크인 D-N 자동 청구 vs 운영자 수동. FX 확정 트리거가 여기서 결정됨.
3. **취소·환불 시 다통화 반환 FX** — 낸 통화로 낸 금액 반환 vs VND 기준 재환산(ADR-0047 정책 + 취소규정 정합).
4. **USD** 동일 적용 범위(계좌 미운영 → 현 수동 유지 여부).

## 9. 범위

- **이번:** 설계 확정(이 ADR). 원칙·모델·스키마 골격.
- **다음 스프린트:** §8 결정 → 계약 초안(docs/contracts) → BE(스키마·결제 API·LEDGER)·FE(/p·확정 UX)·FIN(정산 검증) 구현.
