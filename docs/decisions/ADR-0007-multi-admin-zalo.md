...
단, **테오가 "계정을 하나만 쓰겠다"면** `kind = SYSTEM_BOT`인 테오 계정을 채팅에도 쓰는 단일 계정 모드도 가능하다(풀에서 `"__system__"` 인스턴스를 테오의 개인 스코프로도 노출). 그러나 위 1·2 리스크 때문에 **분리를 기본 권고**하고, 단일 계정 모드는 테오 결정 시 옵션으로 남긴다. → **검증 필요: 테오가 계정 2개(시스템 전용 + 개인)를 운용할 의사가 있는지 확인.** (Phase 1엔 시스템봇=테오, 개인 채팅 관리자=테오 1명일 수 있어 실질 계정 1~2개.)
