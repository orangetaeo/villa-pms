...
- **브레이크포인트 1024px(lg)**: ≥1024 고정 사이드바 / <1024 상단 헤더 + 햄버거 드로어. 하단 탭은 미도입(메뉴 9개 — 오픈 후 사용 패턴 보고, IDEAS)
  - **[2026-06-22 갱신]** 운영자 「빌라별 공실 보드」(/availability, T-admin-availability-board) 추가로 사이드바 **10메뉴**로 확장. 순서: 대시보드/예약/제안/빌라/**공실 보드**/청소 검수/정산/메시지/사용자/설정 (빌라 운영군에 배치). DESIGN.md 표준 메뉴 목록도 동일 갱신 대상
