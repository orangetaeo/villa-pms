...
| D6 | 미니바 정산·재고 이동 | **미니바=운영자 재고, 매출도 우리 것**(공급자 직접판매 게스트가 소비해도 우리 매출). 체크아웃에 소모 정산 포함. **전환별 재고 이동**: 다음이 우리 게스트=모자란 만큼 보충 / 다음이 공급자 직접판매=**우리 재고 전량 회수**. 자동화는 미니바 실재고 ADR-0019 Phase 2, F10 Phase A는 체크아웃 정산 블록만. §6.1 |
...
- **체크아웃**: 기준 사진 대조 촬영 → 파손/보증금 처리 → **미니바 소모 정산(D6, 아래 §6.1)** → `status=CHECKED_OUT` → `CleaningTask` 생성 + `isSellable=false`(누가 묵었든 청소 필요).
...
### 6.1 미니바 재고 이동 — 업무 프로세스 (D6, 테오 2026-06-26)

**미니바 = 운영자(우리) 재고.** 공급자 빌라에 우리가 채워 넣는 우리 자산이므로, **미니바 매출은 누가 묵었든(공급자 직접판매 게스트 포함) 우리 것**이고 소모분은 **그 게스트의 체크아웃에서 정산**한다. 공급자 직접판매의 *객실료*는 공급자 100%(D3)지만, *미니바*는 별개로 우리 매출.

핵심은 **예약 전환 시 재고 이동이 "다음 예약의 seller"에 따라 갈린다**는 것:

| 전환 (이전 게스트 체크아웃 → 다음 게스트) | 미니바 처리 |
|---|---|
| 우리 게스트 → **우리 게스트(seller=OPERATOR)** | 이전 게스트 소모분 정산 + **모자란 수량만 보충**(par까지 top-up, `RESTOCK`). 재고는 빌라에 유지 |
| 우리 게스트 → **공급자 직접판매(seller=SUPPLIER)** | 이전 게스트 소모분 정산 + **우리 재고 전량 회수**(`RECOVER`/음수 `ADJUST`) — 우리가 운영하지 않는 판매에 우리 재고를 남겨두지 않음 |
| (체크인 직전) 우리 게스트 입실 | par까지 채워 넣음(빈 빌라면 전량 `RESTOCK`) |

→ 따라서 **공급자 직접판매 게스트의 투숙 중에는 보통 우리 미니바가 없음(전량 회수됨)**. 체크아웃의 미니바 정산 블록은 ① 일반 원칙(소모 시 우리 매출) ② 회수 전 소비된 경우의 안전장치.

**범위/구현:** 이 재고 이동 자동화(빌라별 현재고 `onHandQty`, `MinibarStockMovement`의 `RESTOCK`/`CONSUME`/`RECOVER`, 다음 예약 seller 인지 보충/회수)는 **미니바 실재고 = ADR-0019 §4.1(Phase 2)** 영역이다. **F10 Phase A**에서는 공급자 체크아웃 화면에 **미니바 소모 정산 블록을 포함**(소비 수량×판매가 → 게스트 청구)하되, 전환별 자동 보충/회수 로직은 ADR-0019 Phase 2에서 완성한다. 두 ADR 교차 갱신.
...
| 케이스 | 기대 |
|---|---|
| 공급자가 타인 빌라에 직접예약 생성 시도 | 404 (존재 비노출 — findFirst{id,supplierId} 미스. ID 열거 차단 위해 403 대신 404, T10.2 QA 확정) |
