...
## 디버깅 라운드3 (2026-06-24, 수정 적대검증 + 완결성 비평)

### 수정 완료
- **[HIGH 회귀] representativeRatesBySeason 수정이 공급자 재제출 prefill 깨뜨림**: `edit/page.tsx` prefill도 같은 헬퍼를 써서, HIGH/PEAK 기간 없는 빌라는 prefill이 비어 마법사 제출 잠김(원가 재입력 강요). → prefill에서만 base(LOW) 명시 폴백 복원(`rep[season] ?? rep.LOW`). 표시·경보는 폴백 없음 유지(상반 요구를 호출처에서 분리). 현 프로덕션(전역 HIGH/PEAK 보유)은 미발현이나 엣지 빌라 회귀.

### 보류 (정책/협업 — 추적)
- **[MED i18n] candidates 경고 사유 한국어 하드코딩**: "판매가 미책정"(라운드2 신규)·"요율 미설정(LOW)"(기존)이 vi 로케일 ADMIN에 한국어로 노출(warningsTitle은 vi 있음). 근본해소=candidates가 reason 코드(PRICE_NOT_SET/RATE_NOT_SET)+season 반환→proposal-create가 t()로 매핑→ko/vi 키 추가. **messages/*.json이 타세션 대규모 미커밋(vi 177줄)이라 surgical 추가 위험 → LOC 협업 시 처리**(키 없이 t()만 바꾸면 키명 노출돼 더 나쁨). all-pages-vietnamese-required 부채.

### 결함 없음 확인 (라운드3 완결성 비평)
마진 자동계산 클라/서버 일치(BigInt 정수 동일)·가용성↔요율 독립·다기간 4구간 박별합산 정확(경계 무중복)·시드 단조성/base1개/겹침0·AuditLog 4경로 전부 기록(원가·마진 미노출). candidates 0원 가드 BigInt 0n falsy·통화분기 견고(오제외 없음).
