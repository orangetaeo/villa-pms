...
| 웹챗 게스트 링크 전달(세션↔예약 연결+원클릭 발송) **[승인·구현중]** | docs/plans/webchat-guest-link-share.md + docs/contracts/T-webchat-guest-link-share.md — WebChatSession.bookingId additive·자동 후보(sourcePage g:토큰8자 매칭)·빠른 링크 3종(체크인/부가서비스/영수증, 5언어 사전 템플릿=Gemini 미경유)·핵심 위험=오발송(확인 다이얼로그+revoke 동선+AuditLog) |
