...
| 스테이징(테스트 서버) 환경 구축 **[확정·절차서 작성됨]** | docs/ops/staging-environment-plan.md — 방식 B(완전 별도 Railway 프로젝트) 채택. 「## 실행 체크리스트(확정)」에 A~G 단계(테오 🧑 직접 vs OPS 🤖 구분). 도메인=Railway 기본 *.up.railway.app(Cloudflare 생략)·DB=스테이징 전용(빈 DB 최초 1회 `prisma db push` STAGING URL만·프로덕션 금지)·Zalo=이중 차단(`APP_ENV=staging` 코드 no-op + AppSetting ZALO_OPERATOR_NOTIFY_PAUSED=1)·cron 0개. 안전 플래그 코드 게이트=lib/zalo-runtime.ts sendVia + STAGING 배지. 월 $10~20 |
...
| 제안 통화 3분리(환율 대전제) **[채택·§3Q1 개정]** | docs/decisions/ADR-0047-proposal-currency-billing-vs-display.md — ①원가·정산=항상 VND(진실) / ②청구=제안서당 1개, 생성시점 고정=인보이스(입금받는 통화로 결정) / ③표시참고=항상 "약(≈)". 제안서는 오퍼 동결(박별 환율환산 금지, fxVndPerKrw 스냅샷). **"VND only 확정" 개정**: 정산 VND·청구는 입금통화. 표시규칙=청구 VND일 때만 보조 "약 $/₩". saleCurrency=청구통화, assertSaleAmountColumns=단일성. Phase1 폴백경고 완료. **★계약금/잔금 분할은 ADR-0048로 이관(Q1 개정)** |
| B2C 계약금/잔금 + VND 앵커 다통화 결제 **[정책확정·미구현]** | docs/decisions/ADR-0048-b2c-staged-payment-vnd-anchor.md — **VND=앵커(진실), 각 결제 이벤트마다 그 시점 FX로 원화/달러 확정**(계약금=계약금시점·잔금=잔금청구시점, 다를 수 있음) → 우리 VND 마진 보호·환리스크는 고객 원화총액 변동. ADR-0047 "한국인 KRW 완전고정"을 교정(일시불만 유효). 신규 **BookingPayment**(kind DEPOSIT·BALANCE·REFUND, amountVnd 앵커몫·billingCurrency·amountBilled 확정값·fxVndPerBilling 결제별 스냅샷·status), Booking.totalSaleVnd 앵커 필수화+백필, PartnerReceivable 패턴 B2C 다통화 확장. LEDGER는 각 입금 VND 전기(ADR-0018). **★정책확정(테오 2026-07-24): 계약금 50%(AppSetting B2C_DEPOSIT_RATE_PCT)·잔금 체크인 D-14 자동청구(D-14 이내=100% 선결제)·환불은 낸 통화 낸 금액 그대로 역분개·USD 처음부터(단 계좌 미운영→수동 기록).** 구현=별도 스프린트(계약서→BE·FE·FIN) |
| 블로그 개별 영상 글(category="video") **[제안·설계확정]** | docs/decisions/ADR-0049-blog-video-articles.md + docs/contracts/T-blog-video-articles.md — SeoArticle 재사용 A안(**스키마 마이그레이션 0건**, String 화이트리스트 +"video"), 원천=YoutubeShort(PUBLISHED·ytVideoId·UPLOADED·villaId), 트리거=seo-draft ⑤ 브랜치(topicKey `video-<shortId>` 중복방지·기발행 2건 자동백필), 플레이어=기존 video 블록 YT 임베드 재사용(코드 0줄), 발행자격=video 카테고리 하한 300자+video 블록 필수(isArticlePublishable category 분기). ★메인 sitemap `video:` 확장 금지(네이버 함정)—sitemap-video.xml 확장·GSC만. 실명금지=villa-prep 관문+publicVillaLabel, 회귀테스트 2종 확장. 테오 열린질문 3건(§11) |
...
| 문서 | 내용 |
|---|---|
| ★docs/marketing/video-production-runbook.md | **빌라 투어 쇼츠 제작 런북(정본, 2026-07-23)** — 다음 영상을 한 번에 뽑기 위한 절차 전부. 컷 표 만들기 → 재단 → **로컬 검수(업로드 전 필수)** → 렌더 → 완성본 확인 → 승인·업로드. ★사고 3대 원인(앞 3~4초만 나감 / 이동 컷은 3.5초까지 읽어 다음 컷과 겹침 / 욕실은 변기 등장 시각이 시작점 결정)과 이미 코드로 막아 둔 것 vs 사람이 지켜야 할 체크리스트. 스크립트=`smoke/m1-30cut.mts` |
| docs/marketing/storyboard-m-villa-m1.md | **M villa M1 투어 쇼츠 스토리보드(정본 컷 표, 2026-07-23)** — 원본 540초 워크스루의 30컷 초 단위 목록 + 완급(fast/slow) + 수정 이력 3라운드(변기 회피 7컷·중복 회피). 절차는 위 런북 참조 |
