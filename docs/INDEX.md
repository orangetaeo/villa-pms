...
| 제안 통화 3분리(환율 대전제) **[채택]** | docs/decisions/ADR-0047-proposal-currency-billing-vs-display.md — ①원가·정산=항상 VND(진실) / ②청구=제안서당 1개, 생성시점 고정=인보이스(입금받는 통화로 결정) / ③표시참고=항상 "약(≈)". 제안서는 오퍼 동결(박별 환율환산 금지, fxVndPerKrw 스냅샷). **"VND only 확정" 개정**: 정산 VND·청구는 입금통화. 표시규칙=청구 VND일 때만 보조 "약 $/₩"(KRW청구엔 원화만). saleCurrency=청구통화, assertSaleAmountColumns=단일성. 집중=한국인 KRW 확정가(Phase1 폴백경고 완료). 후속=보조약표시·외국인USD·환율노후 마진버퍼 |
