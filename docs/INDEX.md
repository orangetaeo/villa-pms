...
| B2C 계약금/잔금 + VND 앵커 다통화 결제 **[설계확정·미구현]** | docs/decisions/ADR-0048-b2c-staged-payment-vnd-anchor.md — **VND=앵커(진실), 각 결제 이벤트마다 그 시점 FX로 원화/달러 확정**(계약금=계약금시점·잔금=잔금청구시점, 다를 수 있음) → 우리 VND 마진 보호·환리스크는 고객 원화총액 변동. ADR-0047 "한국인 KRW 완전고정"을 교정(일시불만 유효). 신규 **BookingPayment**(kind DEPOSIT·BALANCE, amountVnd 앵커몫·billingCurrency·amountBilled 확정값·fxVndPerBilling 결제별 스냅샷·status), Booking.totalSaleVnd 앵커 필수화+백필, PartnerReceivable 패턴 B2C 다통화 확장. LEDGER는 각 입금 VND 전기(ADR-0018). ★후속 테오결정=계약금율·잔금청구시점·취소환불 FX·USD범위. 구현=별도 스프린트(계약서→BE·FE·FIN) |
