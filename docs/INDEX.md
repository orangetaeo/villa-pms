...
| 스테이징(테스트 서버) 환경 구축 **[검토 중·미착수]** | docs/ops/staging-environment-plan.md — 프로덕션과 동일한 테스트 서버 검토. 결론: 월 $10~20·구축 반나절, 방식 B(완전 별도 Railway 프로젝트) 추천. 유일 난점=Zalo 알림 분리(별도 계정 QR vs 발송끄기 스위치). 테오 결정 대기 |
