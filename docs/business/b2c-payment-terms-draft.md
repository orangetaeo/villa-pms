...
- 상태: **문구·언어 확정(테오 2026-07-24): 계약금 50% 유지 · 잔금 환율 공시 톤 그대로 · 지원 5언어(ko/vi/en/zh/ru).
  일본어(ja) 불필요.** → 전부 잠금. **P5 구현 착수.**
...
- ✅ 계약금율 **50% 유지**.
- ✅ 잔금 환율 변동 공시 **톤 그대로**.
- ✅ 언어 **확정 = 지원 5언어(ko/vi/en/zh/ru)**. **일본어(ja) 불필요**(테오 2026-07-24) — §2의 [日本語]
  블록은 미사용(참고 보관). `/p`는 ja를 지원하지 않으므로 로케일 신설도 없음.
