...
**결정 1 — 빌라 노출 = 「대표 빌라만 선별 공개」**
- `Villa.publicListed` 기본값 **false**, 운영자가 켠 빌라만 페이지 생성·sitemap 등재
- **전체 빌라 목록/필터 페이지를 만들지 않는다.** 진입 동선은 지역 허브·테마 조합·가이드 글 본문 링크만
- 선별 기준(운영자 판단): 사진·소개문이 충실하고, 공급자와 관계가 안정적이며, 대표성이 있는 빌라
- ⚠ 한계 명시: sitemap.xml은 공개 문서라 **켠 빌라는 경쟁사도 전수 열람 가능**하다. "선별 공개"는 노출 총량을 통제하는 것이지 은닉이 아니다. 켤지 말지를 빌라 단위로 판단하는 것이 유일한 방어선이다
...
| 스프린트 | 범위 | 예상 |
|---|---|---|
| **S0 — 디자인** | DESIGN: 공개 홈 · 가이드 글 상세 · 빌라 상세 3화면 Stitch export (`design/stitch/`) | 1.5일 |
| **S1 — 공개 기반·색인 인프라** | 공개 홈(+PWA start_url 분리·스플래시 제외·회귀), robots/sitemap/feed, IndexNow, 소유확인 메타 3종, 공개 직렬화 봉인 + 누수 테스트 | 3일 |
| **S2 — 가이드 콘텐츠 자동화** | `SeoArticle` 스키마, `seo-draft` cron(주제 풀), admin `/marketing/seo` 승인 큐, **점진 발행 큐(하루 5개 상한)**, 발행 시 IndexNow 핑 | 3일 |
| **S3 — 빌라·지역·조합 (구조)** | 빌라 상세(선별 공개), 지역 허브, 테마 조합(3개 미만 미생성), 내부 링크, JSON-LD, GA4·전환추적·CSP | 2.5일 |
