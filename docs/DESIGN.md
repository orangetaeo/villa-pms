...
- 운영자 표준 사이드바 **9메뉴**(순서 고정, ADR-0003으로 "메시지" 삽입): **대시보드 / 예약 / 제안 / 빌라 / 청소 검수 / 정산 / 메시지 / 사용자 / 설정**
  - 디자인 산출물 중 9메뉴 적용은 b14 1장뿐 — 기존 b1~b13 디자인은 8메뉴 그대로 두고, **Next.js 변환 단계의 공통 Sidebar 컴포넌트에서 일괄 9메뉴로 구현**한다 (화면별 재수정 금지)
