...
- [ ] 수영장 빌라에서 발코니 스킵 제출 → 관리자 검수 뷰에서 수영장 사진이 **수영장 기준** 옆에 표시
- [ ] 청소원 읽기전용 뷰: 슬롯 스킵 제출에서도 공간 라벨 표시(photoSlots 있는 제출)
- [ ] photoSlots 없는 **레거시 제출**은 기존 인덱스 페어링 그대로 (회귀 0)
- [ ] 승인(approve)은 장수 무관 — 기존 동작 유지
- [ ] tsc 0 / 단위 테스트 통과 / next build 통과
- [ ] 누수 체크: 검수 select 화이트리스트 불변(고객명·금액 미포함)
