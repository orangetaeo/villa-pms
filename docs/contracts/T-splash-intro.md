# T-splash-intro — villa-go.net 최초 진입 스플래시 인트로 (핀 드롭, 2.3초)

> 담당: FE(구현) · QA(검증) · 상태: 착수 (2026-07-15) · 세션: 메인 폴더 → worktree 격리

## 1. 목적·배경

- villa-go.net 최초 진입 시 2.3초(하드 상한 2.9초) 브랜드 스플래시 — 로고(지도핀+집)가 3D로 완성되는 인트로. (등장 1.3초 + 완성 로고 홀드 0.7초 + 페이드 0.3초. 테오 지시로 1.6초→2.6초 연장 후 2.3초로 재조정, 2026-07-15)
- 사용자 결정: 연출 3안 중 **1안 핀 드롭** 채택 (DESIGN·FE 회의 완료, 2026-07-15).

## 2. 설계 결정 (회의 요약)

- **연출 (핀 드롭)**: 핀 3D 원근 낙하(0.1–0.5s) → 착지 바운스+링 파동(0.5–0.7s) → 오렌지 지붕점 팝(0.7–0.9s) → "Villa GO" 워드마크+태그라인(0.9–1.3s) → 완성 로고 홀드(1.3–2.0s) → 페이드 아웃(2.0–2.3s). 태그라인: ko `찾던 그 빌라, 여기 있어요` / vi `Villa bạn tìm, có ở đây`.
...
- **접근성**: prefers-reduced-motion=스킵(또는 ≤300ms 정적 페이드), 탭/키 입력 즉시 스킵, 오버레이 `aria-hidden` + 포커스 트랩 금지, `visibilitychange`·2.9s setTimeout 하드 종료.
...
2. 탭/클릭/키 입력 시 즉시 종료. 하드 상한 2.9초 내 무조건 종료.
