...
## 2차 다듬기 (wt/video-polish, 같은 날 자기 리뷰)

1. **오버레이 트림** — satori 1080×1920 캔버스를 sharp `trim`으로 잘라 좌표 지정 overlay.
   결과 픽셀 동일, 렌더 시간 **45% 단축**(23.3s→12.9s 외 2건). 1차에서 올린 인코더 품질의 비용을 여기서 되찾았다.
2. **UNKNOWN 원칙** — 공간·메모 신호가 둘 다 없으면 속도를 건드리지 않는다(1.0).
3. **ETC 단독 1.45배** — 메모로 확신한 이동 컷(1.85)과 분리. 근거의 세기에 배속을 맞춘다.
4. **이동 컷 화면 하한 1.3초**(`minSegmentSecByClip`) — 단, 0.8초 아래 금지(xfade T 전제).
5. **잠재 드리프트 봉인** — `xfadeConcat`이 실제 전환 길이를 반환 → 재동기화가 상수 대신 그 값 사용.
6. `alimiter`(amix 합산 클리핑 방지) · 포스터 `-q:v 2` · 마법사 공간 미지정 경고.
