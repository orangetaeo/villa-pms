...
1. 권한 누수 4종: ① 공급자가 타인 빌라 직접예약 POST → **404(존재 비노출 — findFirst{id,supplierId} 미스, ID 열거 차단. 403보다 안전, T10.2 QA 확정)** ② 직접예약 API 응답·타임라인 공급자뷰에 운영자 salePriceKrw/margin → 0건 ③ 운영자 홀드 날짜에 공급자 직접예약 → 409(상세 비노출) ④ 정산 집계 seller=SUPPLIER 혼입 → 0건
