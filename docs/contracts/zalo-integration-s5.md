...
#### A6-2 alias (별명) — 거의 무비용
- **신규 코드 최소**. villa는 이미 `ZaloConversation.nickname` + `PATCH /api/zalo/conversations/[id]` `SET_NICKNAME` 액션 보유(app/api/zalo/conversations/[id]/route.ts:170~197, AuditLog 기록). villa 내부 alias 편집·표시·정본은 **이미 완비** — 추가 구현 없음.
- **정리할 1가지(아래 ③에서 확정)**: Nike↔villa alias **동기화 방향**. 결론 = **읽기 단방향**(villa nickname을 정본으로, Nike가 S2 읽기(`/ext/threads` DTO의 `nickname` 필드)로 **표시만**). Nike→villa로 nickname을 **쓰는** ext 경로는 신설하지 **않는다**(MVP 무비용·누수면 최소). 즉 A6-2의 villa 측 구현 = **0건(기존 SET_NICKNAME·S2 DTO nickname 재사용)**, Nike B5가 자기 setAlias UI를 "villa 별명 표시"로 전환(또는 자기 인메모리 alias 유지하되 villa nickname 우선 표시).
  - **(대안 — 본 계약 비채택, 향후 확장 명시)** 양방향 쓰기가 필요하면 ext에 `POST /ext/conversations/{id}/nickname`(시크릿 게이트 + 테오 스코프)을 신설해 SET_NICKNAME 로직을 위임. 본 MVP는 읽기 단방향으로 확정(쓰기 ext 신설 시 본 계약 개정 필요).
...
- **B5-alias**: 테오 setAlias(changeAlias L607 / removeAlias L631)는 **villa nickname 정본을 표시**하도록 전환 — S2 `/ext/threads` 응답의 `nickname`을 우선 표시. Nike 자체 alias 쓰기 호출은 테오에 대해 비활성(또는 villa nickname으로 덮어쓰기). zca-js `changeFriendAlias`/`removeFriendAlias` 직접 호출은 테오 세션이 Nike에 없으므로(S1 B1) 어차피 불가 — 표시 위임만.
...
- **Nike→villa nickname 쓰기 ext 경로** → 범위 밖(읽기 단방향 확정). 필요 시 본 계약 개정.
- **villa 스키마 변경 0건**(forward=발송 함수+ext kind, alias=기존 nickname·SET_NICKNAME 재사용, STT=기존 translatedText 재사용. 신규 컬럼·마이그레이션 0).
...
- [ ] **음성 수신 STT 표시**: 상대가 음성 메시지 발신 → villa가 voice로 저장(msgType "voice") → STT→ko 번역이 translatedText에 채워져 villa /messages에서 음성 내용 텍스트 표시. S2 재push로 Nike에도 STT 텍스트 실시간 반영.
