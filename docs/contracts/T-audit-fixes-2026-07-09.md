...
3. P2-2 (QA 정정 반영): agreementSignedAt이 null이면 /g 로더 반환값의 **wifiPassword만 null** — wifiSsid·주소는 서명 전 노출이 기존 출시 설계라 유지(회귀 방지). 서명 후에는 기존대로 노출 + 서명 API 응답으로 리로드 없이 즉시 표시
