...
export default function GuestFlow(props: GuestFlowProps) {
  const { token, lang, booking, amenityGroups, minibar, agreement, receiptHref } = props;
      ...
      {step === 0 && (
        <>
          <main className="flex-grow px-6 py-7 space-y-7">
            {/* 정산 내역(영수증) 진입점 — 체크아웃 완료 시에만(receiptHref) 최상단 노출 */}
            {receiptHref && (
              <a
                href={receiptHref}
                className="flex items-center gap-3 bg-slate-900 text-white rounded-2xl p-4 shadow-lg active:scale-[0.98] transition-transform"
              >
