...
import { MINIBAR_VND_DIGITS, generateMinibarItemKey } from "@/lib/minibar";
...
export async function GET() {
  ...
  return NextResponse.json({
    items: items.map((m) => ({ ...m, unitPriceVnd: m.unitPriceVnd.toString() })),
  });
}
...
export async function POST(req: Request) {
  ...
  // 감사 로그 — 판매가 스냅샷 포함(BigInt → 문자열). 데이터 변경 절대 규칙.
  await writeAuditLog({
    userId,
    action: "CREATE",
    entity: "MinibarItem",
  ...
  return NextResponse.json({ item: { ...created, unitPriceVnd: created.unitPriceVnd.toString() } });
}
