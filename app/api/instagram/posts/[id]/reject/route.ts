// POST /api/instagram/posts/[id]/reject — 초안 반려 (PENDING_APPROVAL·QUEUED) → CANCELLED. admin.
...
const REJECTABLE = [IgPostStatus.PENDING_APPROVAL, IgPostStatus.QUEUED];
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const res = await prisma.instagramPost.updateMany({
    ...
    data: { status: IgPostStatus.CANCELLED, failReason: reason ?? null },
  });
  if (res.count === 0) {
  ...
  return NextResponse.json({ ok: true, status: IgPostStatus.CANCELLED });
}
