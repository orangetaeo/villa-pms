...
// 파트너(여행사·랜드사) 인앱 알림센터 벨 (T-partner-workflow-gaps ①) — vendor-notification-bell 미러.
//   GET /api/partner/notifications(목록·미읽음 수), POST /read(읽음 처리).
//   ★ 누수: API가 가격 없는 InAppNotification만 내려줌(마진·원가·KRW 없음). 이 컴포넌트는 그 shape만 사용.
//   SSE 아님 — 마운트 시 + 주기 폴링(POLL_MS)으로 미읽음 갱신. 시트 열 때 read 호출해 0으로.
//   i18n: partner.notif.* (파트너 레이아웃이 partner 네임스페이스를 직렬화 — 신규 네임스페이스 금지)
...
export default function PartnerNotificationBell() {
  const t = useTranslations("partner.notif");
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      ...
      if ((data.unread ?? 0) > 0) {
        setUnread(0);
        void fetch("/api/partner/notifications/read", {
          method: "POST",
  ...
  return (
    <>
      <button
        ...
        className="relative flex h-9 w-9 items-center justify-center rounded-full text-neutral-600 active:scale-95"
      >
