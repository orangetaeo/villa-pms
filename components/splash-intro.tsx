 ...
 *  - 정상 종료: 2.3s(등장 1.3s + 완성 로고 홀드 0.7s + 페이드 0.3s)
 *  - 하드 상한: 2.9s
...
export default function SplashIntro({ tagline }: { tagline: string }) {
  ...
  useEffect(() => {
    ...
    // 정상 종료(애니메이션 끝, 완성 홀드 후 2.3s) + 하드 상한 이중 방어.
    timers.push(window.setTimeout(finish, 2300));
    timers.push(window.setTimeout(finish, 2900));
