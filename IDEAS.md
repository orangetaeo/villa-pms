...
- **멀티채널 상담 통합: 표준 메시지 모델 + 채널 어댑터** (2026-07-13 테오) — 해외 소비자 확대 시 WhatsApp(카자흐·홍콩·인도)·Telegram(러시아)·LINE(대만) 등 채널 추가가 필요해지는데, 채널별 UI를 만들지 말고 **내부 표준 메시지 모델(텍스트·이미지·파일·시스템상태 최소셋)만 렌더링하는 단일 인박스 + 채널별 어댑터가 포맷·정책(WhatsApp 24h 세션 등) 차이를 흡수**하는 구조로. UI에 노출하는 채널 차이는 채널 뱃지·발신 가능 여부 2가지만. 당장 할 일: `/messages` 인박스 코드에 Zalo 전용 가정을 깊이 박지 않기. 착수 전 단계는 제안 페이지에 메신저 딥링크 버튼으로 수요 검증 먼저.
