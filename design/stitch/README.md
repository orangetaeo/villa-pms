...
| a-direct-booking | 직접 예약 기록 바텀시트 (F10 Phase A, ADR-0021 §6) | (로컬 컴포지션 — a3·a14 계승, Stitch 화면 없음) |
| a-supplier-checkin | 공급자 직접 체크인 검수 (F10 D5, ADR-0021 §6) | (로컬 컴포지션 — b3 vi 라이트 재해석, Stitch 화면 없음) |
| a-supplier-checkout | 공급자 직접 체크아웃 검수 (F10 D5, ADR-0021 §6) | (로컬 컴포지션 — b4 vi 라이트 재해석, Stitch 화면 없음) |
...
## 로고 — Villa Go 브랜드 (3컨셉) (DESIGN 에이전트, 2026-06-25)

신규 브랜드명 **"Villa Go"**(빌라 임대 + 여행/이동 Go) 워드마크 기반 로고 프레젠테이션 보드 3종.
폴더: `design/stitch/logo-villa-go/` — 각 컨셉 폴더에 `index.html` + `screenshot.png`(1440 @2x full-page) + `NOTES.md`.
라이트 디자인 시스템(`assets/16166333700622029900`, Be Vietnam Pro·틸 #0D9488·샌드 #F59E0B) 적용. 로고 아트는 전부 인라인 SVG(사진·외부 이미지 0건). 공통 폰트 폴백(Noto Sans KR)·`word-break: keep-all` 주입.

| 폴더 | 컨셉 | 산출 방식 | Stitch screen ID |
|---|---|---|---|
| logo-villa-go/concept-a | A — 워드마크 + 빌라 루프라인(삼각 지붕+리지+샌드 해 점). 절제·신뢰감 | 로컬 컴포지션(B 쉘 재사용) | (클라우드 없음) |
| logo-villa-go/concept-b | B — 로케이션 핀 = 빌라(핀 머리에 빌라 흰 녹아웃 + 샌드 해 점). 식별력·앱아이콘 최강 | **Stitch 클라우드 생성** + 로컬 보정 | `3d6148790c5b40eb8851ae396b2d53b4` |
| logo-villa-go/concept-c | C — 떠오르는 해 + 빌라(지붕 뒤 샌드 해 + 수평선). 푸꾸옥 리조트 감성 | 로컬 컴포지션(B 쉘 재사용) | (클라우드 없음, generate 타임아웃·list 캐시 고정) |

- 각 보드 구성(한 캔버스): Hero 풀 lockup · Horizontal · Stacked · Monochrome(틸 단색) · App icon 3사이즈 · On-dark(네이비 #0F172A) · 컬러칩 3종(Ink/Teal/Sand HEX).
- 자가검토(4기준: 브랜드 적합성/가독성·확장성/단순성/일관성) 결과 **추천 1순위 = Concept B** (마크 식별력·앱 아이콘·사업 본질 결합·클라우드 원본 확보). 상세 코멘트는 각 폴더 NOTES.md.
- 잔존: A·C는 클라우드 미반영(로컬 canonical). 추후 Stitch 웹 재생성 시 본 README의 컨셉 설명을 프롬프트로 사용.
