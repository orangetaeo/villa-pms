...
- 아이콘: `.ico` 3개(🛏/👨‍👩‍👧/🏖) → satori에서 SVG `<img>`로 교체(§0 ⚠).
...
| **완성도** | 중상 | 타이포 위계 명확(eyebrow<headline, label<copy), 스크림으로 대비 확보, 간격 일관. 잔여: 이모지→SVG 교체 전까지 info 아이콘 정렬은 폰트 의존. |
...
**BE 변환 인계 시 확인 요청 3건**:
1. 이모지→모노크롬 SVG 아이콘 교체(info) — 아이콘 에셋 준비.
